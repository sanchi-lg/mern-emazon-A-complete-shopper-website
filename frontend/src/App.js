...
import { cart, getcat } from './components/ser';
...
export class App extends Component {
  constructor(props) {
    super(props)
    this.state = {
       cnum:0,spos:true,iRef:React.createRef(),sc:"",sca:[]
    }
...
cart(localStorage.getItem('uid'))
.then(res=>{
  this.setState({cnum:res.data})
})
  }
  }
...
<div className="grid-container">
 ...
 <input type="text" placeholder="Search" name="search"  onKeyPress={this.state.iRef.current.shandle} /><button onClick={this.state.iRef.current.search}>{<AiOutlineSearch/>}</button>
    ...
    <Link to="/wishlist" title="wishlist" className="wish">{<FaHeart/>}</Link>}
        {!localStorage.getItem("aid")&&this.state.cnum!=0&&
...
<Link to="/cart" >Cart<span>{<BiCart/>}</span></Link>}
  ...
  <aside className="sidebar">
    <h2 style={{marginLeft:"20px",color:"purple",maxWidth:"90%",overflow:"hidden",textOverflow:"ellipsis"}}> {(localStorage.getItem('uid')!=undefined||localStorage.getItem('aid')!=undefined)?<span style={{color:"purple",fontWeight:"lighter",
    fontSize:"medium"}} > {localStorage.getItem('uid')!=undefined?<span><span style={{fontSize:"large"}}>Hi  </span >{localStorage.getItem('uname')}</span>:<span>Admin Portal</span>}</span>:
<Link style={{ textDecoration:"none",color:"purple",fontWeight:"lighter",fontSize:"large"}} 
to="usersignin">Sign-in</Link>
...
<li>
 ...
 {this.state.sc=="kids"&&
      ...
      {/* <li>
        <a href="/productbycategory/men">Men</a>
      </li>
      <li>
        <a href="/productbycategory/women">Women</a>
      </li>
       <li>
        <a href="/productbygategory/kids">Kids</a>
